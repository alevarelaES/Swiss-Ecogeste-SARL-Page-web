...
const SubventionCard = ({ icon: Icon, title, description, delay }: { icon: any, title: string, description: string, delay: number }) => (
    <Reveal delay={delay}>
        <div className="bg-white p-8 rounded-lg border-b-4 border-amber-400 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all h-full flex flex-col items-center text-center">
            ...
            <a href="#contact" className="inline-block bg-green-500 hover:bg-amber-500 text-white px-6 py-2 rounded-full text-sm font-bold transition-colors">
                Formulaire
            </a>
        </div>
    </Reveal>
    ...
    return (
        <section className="py-16 relative bg-gray-200 overflow-hidden">
            ...
            <div className="absolute inset-0">
                <div className="absolute top-10 right-20 w-64 h-64 bg-green-100 rounded-full blur-3xl opacity-30"></div>
                <div className="absolute bottom-20 left-10 w-96 h-96 bg-green-200 rounded-full blur-3xl opacity-20"></div>
            </div>

            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <Reveal>
                    <div className="text-center mb-12">
                        <span className="text-amber-500 font-bold tracking-wider uppercase text-lg bg-amber-50 px-4 py-2 rounded-full inline-block">Subventions</span>
